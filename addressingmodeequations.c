...
int main()
{
  ...
  for(int flag=1;flag<=MAX_FLAG;flag*=2) {
      ...
      for(int m=0;m<512;m++)
	{
	  ...
	  for(int v=0;v<512;v++) {
	    ...
	    if (mv!=last_mv) {
	      ...
	      for(int i=0;i<512;i++) {
		if ((i&m)==v) {
		  if (length==-1) length=modes[i];
		  else if (length!=modes[i]) { matches=-1; break; }
      ...
      printf("Flag $%04X, Rule #%d: OPCODE & $%03x = $%03x -> flag is %s (covers %d/%d remaining instructions).\n",
	     flag,rule_count,best_m,best_v,flagname(flag),best_matches,remaining);
    ...
    printf("Verifying that %d rules correctly classify lengths of all instructions.\n",
	   rule_count);
    for(int i=0;i<512;i++) {
    int actual_value=modes[i]&flag;
    int predicted_value=-1;
    for(int r=0;r<rule_count;r++)
      {
	if ((i&rules[r].m)==rules[r].v) {
	  if (predicted_value==-1) predicted_value=rules[r].len;
	  else if (predicted_value!=rules[r].len) {
